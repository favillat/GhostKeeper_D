...
class utils:
    ...
    '''
    def LoadSprite(self,NAME="bg", SCALE=0,SCALEBY = 2, KEY=(0,0,0), ALPHA=255):
        try:
            img = py.image.load("./sprites/"+str(NAME)+'.png').convert()

            #img = py.transform.scale(img,(SCALEIND[0],SCALEIND[1]))
            if SCALE > 0:
                img = py.transform.scale(img,(SCALE,SCALE))
            else:
...
class SpriteManager:
    def __init__(self):
        ...
        self.ssDimensions = 3
        self.spriteSizes = 30
        self.animations = {} #{ Animation Name: [[Sprite1,Sprite2,Sprite3]] }
